...
function buildTemplateElements(template) {
  // KEY FIX: if template already has saved customElements, use them directly
  if (template?.customElements && Array.isArray(template.customElements) && template.customElements.length > 0) {
    return template.customElements;
  }
  ...
  if (template.heading) push({
    id: 'tpl-heading', type: 'text', content: template.heading,
  ...
  if (template.subHeading) push({
    id: 'tpl-subheading', type: 'text', content: template.subHeading,
  ...
  push({
    id: 'tpl-recipient', type: 'text', content: '{name}',
  ...
  if (template.descriptionText) push({
    id: 'tpl-description', type: 'text', content: template.descriptionText,
  ...
  push({
    id: 'tpl-event', type: 'text', content: '{event}',
  ...
  push({
    id: 'tpl-footer', type: 'text', content: `${template.organizerName || 'Event Organizer'} • {date}`,
  ...
  if (template.logo) push({
    id: 'tpl-logo', type: 'image', src: template.logo,
      ...
      <div className="bg-gray-800 rounded-xl shadow-2xl w-full sm:max-w-md border border-gray-700">
        ...
        <div className="p-5 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {[
                ...
                <button key={f.id} onClick={() => setFormat(f.id)}
                  className={`p-3 rounded-lg border text-left transition ${format === f.id
          ...
          <div className="space-y-2">
            <button onClick={handleSingleExport} disabled={exporting}
              className="w-full flex items-center justify-center gap-2 py-2.5 bg-gray-700 hover:bg-gray-600 text-white text-sm font-semibold rounded-lg transition disabled:opacity-50">
              {exporting ? <Loader size={16} className="animate-spin" /> : <Download size={16} />}
              Download Preview ({format.toUpperCase()})
            </button>

            {hasRecipients && (
              <button onClick={handleBulkExport} disabled={exporting || format === 'print-png'}
...
export default function CanvasCertificateEditor({
  ...
  // ── KEY FIX: build store once, load elements BEFORE saveToHistory ──────────
  const [editorStore] = useState(() => {
    ...
    // Step 2: load designConfig from template.designConfig if it was previously saved
    const designConfig = template?.designConfig || {};

    // Step 3: load both in one call so history captures the complete initial state
    store.getState().loadState({ elements, designConfig });

    // Step 4: save ONE initial history snapshot (loadState resets history to [])
  ...
  // ── KEY FIX: read elements directly from store at save time ───────────────
  // Don't rely on storeState (which is async/subscribed) — call getState() directly
  const handleSave = async () => {
    const freshState = editorStore.getState().serializeState();

    console.log('[Save] elements count:', freshState.elements.length);
    console.log('[Save] designConfig:', freshState.designConfig);

    await onSave({
      ...template,                          // base template fields (heading, colors, etc.)
      ...freshState.designConfig,           // top-level bg/border fields for legacy HTML generator
      designConfig: freshState.designConfig, // nested designConfig for canvas editor reload
      customElements: freshState.elements,   // ← THE critical array
    });
  };
  ...
  const addText = () => {
    ...
    editorStore.getState().addElement({
      ...
      fontSize: 28, color: '#333333', fontFamily: 'Georgia, serif', fontWeight: 'normal',
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => {
        ...
        editorStore.getState().addElement({
          ...
          x: width / 2 - 80, y: height / 2 - 80, width: 160, height: 160,
  ...
  const addShape = (shapeType) => {
    ...
    editorStore.getState().addElement({
      ...
      width: 120, height: shapeType === 'line' ? 4 : 120,
      fillColor: '#D4A574', strokeColor: '#333333', strokeWidth: 2, opacity: 1,
    });
  };

  const addTransparentShape = (shapeType) => {
    const { width = 1050, height = 744 } = storeState.designConfig;
    editorStore.getState().addElement({
      type: 'shape', shapeType,
  ...
  const addQRCode = async () => {
    try {
      ...
      editorStore.getState().addElement({
        type: 'qrcode', src: qrUrl,
        x: width / 2 - 60, y: height / 2 - 60,
        width: 120, height: 120,
        qrText: '{name}-{code}', objectFit: 'contain',
      });
    } catch (err) {
      ...
      <div className="flex flex-1 pt-12">
        ...
        <div className="w-60 bg-gray-800 border-r border-gray-700 flex flex-col flex-shrink-0">
          <div className="flex border-b border-gray-700 flex-shrink-0">
            {[
              { key: 'tools',  label: 'Tools',  icon: <Plus size={13} /> },
              { key: 'layers', label: 'Layers', icon: <Layers size={13} /> },
            ].map(tab => (
          ...
          {activeLeftTab === 'tools' && (
            <div className="flex-1 overflow-y-auto p-3 space-y-1">
              ...
              {[
                ...
                { type: 'triangle',  label: 'Triangle',  Icon: Triangle, color: 'text-orange-400' },
              ...
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider px-1 pt-3 pb-1">Transparent Elements</p>
              <p className="text-xs text-gray-600 px-1 pb-1">Outline-only, semi-transparent</p>
              {[
                { type: 'rectangle', label: 'Outline Rectangle', Icon: Square,   color: 'text-blue-300'   },
                { type: 'circle',    label: 'Outline Circle',    Icon: Circle,   color: 'text-pink-300'   },
                { type: 'line',      label: 'Decorative Line',   Icon: Minus,    color: 'text-yellow-300' },
                { type: 'triangle',  label: 'Outline Triangle',  Icon: Triangle, color: 'text-orange-300' },
              ].map(({ type, label, Icon, color }) => (
                <button key={`t-${type}`} onClick={() => addTransparentShape(type)}
                  className="w-full flex items-center gap-2.5 px-3 py-2 bg-gray-700/50 hover:bg-gray-600 rounded text-sm text-gray-300 border border-gray-700 border-dashed transition">
                  <Icon size={15} className={color} /> {label}
                </button>
              ))}
          ...
          {selectedElement && (
            <>
              <div className="p-3 border-b border-gray-700 flex-shrink-0 flex items-center justify-between">
                ...
                <button onClick={() => editorStore.getState().clearSelection()}
