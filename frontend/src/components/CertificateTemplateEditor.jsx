...
import { getTemplateWithDecorations, getAllTemplatesWithDecorations } from '../utils/prebuiltTemplates';

/**
 * Enhanced Certificate Template Editor
 * - Full template rendering with decorations
 * - Inline editable text elements
 * - Locked decorative shapes
 * - Shared rendering engine between preview and editor
...
export default function CertificateTemplateEditor({
  ...
  // Build combined template data with all styling
  ...
  const handleFontSizeChange = (newSize) => {
    if (currentElement && currentElement.key.includes('FontSize')) {
      handleElementChange(currentElement.key, newSize);
    } else if (currentElement) {
      const fontSizeKey = currentElement.key === 'heading' ? 'headingFontSize' : `${currentElement.key}FontSize`;
      handleElementChange(fontSizeKey, newSize);
    }
  };
  ...
  // Get font size for current element
  ...
  // Render the editable canvas with shared renderer
  const CanvasContent = () => (
      ...
      <div className="w-96 bg-white border-r border-gray-200 flex flex-col overflow-y-auto shadow-sm">
        ...
        {currentElement && (
          <div className="p-6 flex-1 overflow-y-auto space-y-5">
            ...
            {/* Text Alignment */}
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-900">Alignment</label>
              <div className="flex gap-2">
                {[
                  ...
                  <button
                    key={value}
                    className="flex-1 px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors text-gray-700"
                  >
