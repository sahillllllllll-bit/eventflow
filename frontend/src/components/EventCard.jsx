...
const EventCard = ({ event, onClick, onDelete, onShare }) => {
  ...
  return (
    <div onClick={onClick} className="bg-surface border border-surface-overlay rounded-lg hover:border-brand/50 transition overflow-hidden">
      {/* Cover Image */}
      {event.coverImage && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 mb-4 text-xs">
        ...
        <div className="p-2 bg-surface-overlay rounded">
          ...
          <p className="font-semibold text-white text-xs">{formatEventDate(event.date)}</p>
        </div>
      </div>
